...
Boston housing dataset has 489 data points with 4 variables each.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/opt/conda/lib/python3.6/site-packages/sklearn/learning\_curve.py:22: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the functions are moved. This module will be removed in 0.20
  DeprecationWarning)
...
\texttt{prices}. - Store each calculation in their respective variable.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}2}]:} \PY{c+c1}{\PYZsh{} TODO: Minimum price of the data}
...
\texttt{\textquotesingle{}PTRATIO\textquotesingle{}} value of 15?
    ...
    \textbf{Answer: } 1. As the value of ``PM'' increases, the value of
``MEDV'' increases. If some home has 6 rooms, and homes in the
neighborhood has 7 rooms, then home with 6 rooms will be cheaper. 2. As
the value of ``LSTAT'' increases, the value of ``MEDV'' decreases. A
neighborhood that has an ``LSTAT'' value of 15 have home prices be worth
more than a neighborhood that has an ``LSTAT'' value of 20. If the
working poor in the district are few, housing prices in this area are
more expensive. 3. As the value of ``PTRATIO'' increases, the value of
``MEDV'' decreases. A lower ``PTRATIO'' resulting in less attention
dedicated to each student (it is state schools). One would expect a
lower price given a high ``PTRATIO'' (it is a private school) due to a
lower demand for houses in such areas.
...
to the \texttt{score} variable.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}3}]:} \PY{c+c1}{\PYZsh{} TODO: Import \PYZsq{}r2\PYZus{}score\PYZsq{}}
...
\end{Verbatim}
    ...
    \hypertarget{question-2---goodness-of-fit}{%
...
and calculate this model's coefficient of determination.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}4}]:} \PY{c+c1}{\PYZsh{} Calculate the performance of this model}
...
the variance in Y is predictable from X.
    ...
    \textbf{Answer:} This model has successfully captured the variation of
the target variable. The closer the value of the coefficient to 1, the
stronger the dependence. And our model has a coefficient of
determination, R2 score of 0.923.

    \hypertarget{implementation-shuffle-and-split-data}{%
...
\texttt{y\_test}.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}5}]:} \PY{c+c1}{\PYZsh{} TODO: Import \PYZsq{}train\PYZus{}test\PYZus{}split\PYZsq{}}
        ...
        \PY{n}{X} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{array}\PY{p}{(}\PY{n}{features}\PY{p}{)}
        \PY{n}{y} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{array}\PY{p}{(}\PY{n}{prices}\PY{p}{)}
        \PY{n}{X\PYZus{}train}\PY{p}{,} \PY{n}{X\PYZus{}test}\PY{p}{,} \PY{n}{y\PYZus{}train}\PY{p}{,} \PY{n}{y\PYZus{}test} \PY{o}{=} \PY{p}{(}\PY{n}{train\PYZus{}test\PYZus{}split} \PY{p}{(}\PY{n}{X}\PY{p}{,} \PY{n}{y}\PY{p}{,} \PY{n}{test\PYZus{}size} \PY{o}{=} \PY{l+m+mf}{0.20}\PY{p}{,} \PY{n}{train\PYZus{}size}\PY{o}{=}\PY{l+m+mf}{0.80}\PY{p}{,} \PY{n}{random\PYZus{}state} \PY{o}{=} \PY{l+m+mi}{20}\PY{p}{)}\PY{p}{)}
        \PY{c+c1}{\PYZsh{} Success}
...
Training and testing split was successful.
    ...
    \hypertarget{question-3---training-and-testing}{%
...
upon how splits on data is done.
    ...
    \textbf{Answer: } When we train and test on the same data we maximizing
training accuracy rewards overly complex models that overfit the
training data. We are not able to generalize well. So we splitting a
dataset into some ratio of training and testing subsets and it mean that
the model can be trained and tested on different data.Then testing
accuracy is a better estimate.
...
question.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}6}]:} \PY{c+c1}{\PYZsh{} Produce learning curves for varying training set sizes and maximum depths}
...
\end{Verbatim}
    ...
    \hypertarget{question-4---learning-the-data}{%
...
curves are converging.
    ...
    \textbf{Answer: } max\_depth=3. In this graph model fits and generalizes
well. The score of the testing curve increases when testing points
added. When training points added, the score of the training curve
decreases. Having more points will not benefit the accuracy of the
predictions since the score values are converging to 0.8.

    \hypertarget{complexity-curves}{%
...
two questions Q5 and Q6. **
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}7}]:} \PY{n}{vs}\PY{o}{.}\PY{n}{ModelComplexity}\PY{p}{(}\PY{n}{X\PYZus{}train}\PY{p}{,} \PY{n}{y\PYZus{}train}\PY{p}{)}
\end{Verbatim}
    ...
    \hypertarget{question-5---bias-variance-tradeoff}{%
...
the tradeoff.
    ...
    \textbf{Answer: } When the model is trained with a maximum depth of 1,
the model suffer from high bias. In this case the model has a small gap
between the training and testing scores. And testing and traing scores
are low. The model can not fit data well because she is not complex
enough. When the model is trained with a maximum depth of 10, the model
suffer from high variance. The training score is high (R2=1), but the
testing score is low.The model has a substantial gap between the
training and testing scores (about 0.3).Model can not generalize well.
...
the fewest assumptions should be selected.''
    ...
    \textbf{Answer: } Maximum depth of 4 results in a model that best
generalizes to unseen data. In this case the validation score is the
highest and the model has a small gap between the training and
validation scores.As the depth increases, the validation score deсreases
and gap between the training and validation scores increases.
...
a parameter in a model that can be optimized using this approach.
    ...
    \textbf{Answer: } The grid search method is an exhaustive search which
using each combination of the provided hyperparameter values to find the
best model. The grid search algorithm should be guided by a certain
performance metric, measured by cross-validation on the training set .It
helps the learning algorithm to find the proper parameters.
...
for your answer.
    ...
    \textbf{Answer: } 1. In the k-fold cross-validation approach, the
training set is split into k smaller sets. For each of the k ``folds''
model is trained using k-1 of the folds as training data, the resulting
model is validated on the remaining part of the data. Each of the k
subsamples used exactly once as the validation data. The performance
measure reported by k-fold cross-validation is then the average of the
values computed in the loop. 2. The benefits of k-fold cross-validation:
more reliable estimate of performance, the variance is decreases. When
evaluating hyperparameters for estimators, there is still a risk of
overfitting on the test set - without k-fold cross-validation the risk
is higher that grid search will select hyper-parameter value
combinations that perform very well on a specific training test split
but poorly otherwise. Without a cross-validation set, it will be a high
variance (when we run grid-search, we would have different sets of
optimal parameters).
...
\texttt{\textquotesingle{}grid\textquotesingle{}} variable.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}8}]:} \PY{c+c1}{\PYZsh{} TODO: Import \PYZsq{}make\PYZus{}scorer\PYZsq{}, \PYZsq{}DecisionTreeRegressor\PYZsq{}, and \PYZsq{}GridSearchCV\PYZsq{}}
...
variable is unknown --- such as data the model was not trained on.
    ...
    \hypertarget{question-9---optimal-model}{%
...
training data and produce an optimal model.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}9}]:} \PY{c+c1}{\PYZsh{} Fit the training data to the model using grid search}
...
\textbf{Answer: } `max\_depth' is 4 for the optimal model. The max depth
is the same as my guess (in question 6).
...
for each client's home.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}10}]:} \PY{c+c1}{\PYZsh{} Produce a matrix for client data}
...
\$306,337.50. For the client 1,the price is is lower than the average
housing price in Boston and is also within the one standard deviation.
The price seems reasonable because the features RM, LSTAT, and PTRATIO
are also between the other two houses.

Predicted selling price for Client 2's home: \$224,342.55. For the
client 2, the price is lower than the average housing price in Boston
and is also in between one and two standard deviation. The house has
less rooms than the others, and has higher LSTAT and PTRATIO feature.The
price seems reasonable because all these factors will decrease the
price.

Predicted selling price for Client 3's home: \$930,490.91. And the house
price for the client 3 is approaches the maximum price in the data set.
The price seems reasonable given the low level of poverty and PTRATIO
with a large number of rooms (the highest RM).
...
trained on.}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}11}]:} \PY{n}{vs}\PY{o}{.}\PY{n}{PredictTrials}\PY{p}{(}\PY{n}{features}\PY{p}{,} \PY{n}{prices}\PY{p}{,} \PY{n}{fit\PYZus{}model}\PY{p}{,} \PY{n}{client\PYZus{}data}\PY{p}{)}
\end{Verbatim}
...
Range in prices: \$73,357.39
    ...
    \hypertarget{question-11---applicability}{%
...
\tightlist
\item
  Data that was collected from 1978 is not relevant today. Even though,
  we considered the inflation. Demographics have changed a lot since
  1978.
\item
  The PTRATIO feature (present in the data) may not be very important in
  today's market. Now people get more chances to attend school far from
  the neighborhood. The features present in the data are not sufficient
  to describe a home, we can expand their. For example, we can include
  the distance between the house to the closest food market or mall, how
  close are the main sights of the city, how far from major cities or
  the city center the home is, presence of own swimming pool in the
  territory.\\
\item
  From the sensitivity analysis above, we found that for 10 different
  trials, the predictions are all different, which changes from
  351,577.61 to 424,935.00. The range in prices is 73,357.39. It seems
  that the prediction has large fluctuation.The model is not robust
  enough to make consistent predictions.
\item
  Data collected in an urban city like Boston would not be applicable in
  a rural city as the demographics would change. The rural city will
  have own features.
\item
  It is not fair to judge the price of an individual home based on the
  characteristics of the entire neighborhood. This is a rough estimate
  of the houseThe home can have its own characteristics - the presence
  of a swimming pool, a view, etc.
\end{enumerate}
